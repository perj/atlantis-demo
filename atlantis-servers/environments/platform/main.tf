...
module "platform_atlantis" {
  source = "../../modules/atlantis-server"

  instance_name = "platform"
  repo_allowlist = [
    "gitlab.127.0.0.1.nip.io/root/atlantis-demo",
  ]
...
output "atlantis_url" {
  description = "Platform Atlantis URL"
  value       = module.platform_atlantis.atlantis_url
}

output "webhook_url" {
  description = "Webhook URL to configure in GitLab"
  value       = module.platform_atlantis.webhook_url
}

output "webhook_secret" {
  description = "Webhook secret to configure in GitLab"
  value       = module.platform_atlantis.webhook_secret
  sensitive   = true
}

output "gitlab_webhook_setup" {
  description = "Instructions for configuring the GitLab webhook"
  value       = module.platform_atlantis.gitlab_webhook_setup
}
